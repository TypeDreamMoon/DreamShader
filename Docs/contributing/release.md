...
For the current descriptor — `VersionName` `1.8.0`, `IsBetaVersion` `false`, `Version` `180` —
this yields base `1.8.0`, slug `1.8.0`, tag `v1.8.0`, pre-release `false`, and the archive
`DreamShader-1.8.0.zip`.
...
- The section is found by matching `^##\s+<VersionName>\b`, so the changelog heading must *begin*
  ...
  `## 1.8.0 - 2026-08-21` matches the current `VersionName` `1.8.0`. The failure runs the other way:
...
example `DreamShader 1.8.0`. Assets are the plugin zip followed by every file downloaded from the
