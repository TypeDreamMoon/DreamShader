...
For the current descriptor — `VersionName` `1.9.1`, `IsBetaVersion` `false`, `Version` `191` —
this yields base `1.9.1`, slug `1.9.1`, tag `v1.9.1`, pre-release `false`, and the archive
`DreamShader-1.9.1.zip`.
...
- The section is found by matching `^##\s+<VersionName>\b`, so the changelog heading must *begin*
  ...
  `## 1.9.1 - 2026-09-08` matches the current `VersionName` `1.9.1`. The failure runs the other way:
...
example `DreamShader 1.9.1`. Assets are the plugin zip followed by every file downloaded from the
