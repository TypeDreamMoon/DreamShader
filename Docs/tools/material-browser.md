...
| `InMemoryUntracked` | `◐` green | **compiled in memory** | The asset exists in memory and carries no source hash. Only an asset generated by a plugin version that stamped memory-only builds with the path alone can read this way; current builds stamp the hash in both modes, so this is a legacy state |
