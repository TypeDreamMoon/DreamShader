...
| **Sources ▸ Not managed by DreamShader** | every material and instance under the content roots that no scanned source generates — hand-authored ones, and orphans whose source is gone. See [Unmanaged materials](#unmanaged-materials) |
...
Every `UMaterial` and `UMaterialInstanceConstant` under the content roots (`/Game` and the mount
point of each plugin that ships DreamShader sources) that no scanned source resolves to, so the tab
covers the whole project rather than DreamShader's corner of it. They are found through the asset
registry and **never loaded for listing** — a project with a thousand hand-made materials costs a
registry query, not a thousand loads; a row is loaded only when selected, at which point the
inspector reads its real provenance (an orphan generated asset turns out `Unstamped` or
`Generated`, a hand-made one stays `Foreign`) and folds it back into the row.

| Column | For an unmanaged row |
| :-- | :-- |
| *(glyph)* | `◇` grey |
| **Name** | the asset name, tooltip = the object path |
| **Root** | the mount point — `/Game`, `/PluginName` |
| **Status** | `not managed by DreamShader`, or `material instance · not managed by DreamShader` |
...
| `EphemeralUntracked` | `◐` green | **compiled, Ephemeral** | The asset exists in memory and carries no source hash. Only an asset generated by a plugin version that stamped memory-only builds with the path alone can read this way; current builds stamp the hash in both states, so this is a legacy state |
...
| State | Reads |
| :-- | :-- |
| `Generated` | *generated (matches the last build)* |
| `Tweaked` *(since 1.9.0)* | *generated, with parameter overrides on the instance* — the values are yours, rebuilds go ahead and [put them back](../generation/divergence.md#parameter-overrides-on-a-generated-thincustom-instance) |
| `Unstamped` | *generated (no comparable digest)* |
| `Diverged` | *edited by hand since the last build* — shown in orange, and the only state that refuses a rebuild |
| `Foreign` | *not generated by DreamShader* |

The status bar's **edited by hand** count is `Diverged` alone: a tweaked instance is not in trouble
and does not need finding.
...
- [Project settings](../settings/project.md) — *Material Instance Subfolder* and the Ephemeral visibility toggle
